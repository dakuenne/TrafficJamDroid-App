...
public class UpdateDBTask extends AsyncTask<Context, Void, Void> {
	...
	protected Void doInBackground(Context... arg0) {
		try {
			final BoundingBox bbox = LocalViewContainer.getInstance()
					.getMapView().getBoundingBox();
			final int minLat = (int) (bbox.getLatSouth() / 1E6);
			final int maxLat = (int) (bbox.getLatNorth() / 1E6);
			final int minLng = (int) (bbox.getLonEast() / 1E6);
			final int maxLng = (int) (bbox.getLonWest() / 1E6);
